...
class RNN(nn.Module):
    ...
    def forward(self, nn_input, hidden):
        ...
        out = self.fc(out)
        ##sig = self.sig(out)
        sig = out.view(batch_size,-1,self.output_size)
        sig = sig[:,-1]
...
# **Answer:** 
# i just made many tries till the loss becomes close to the required one but at first i use sigmoid function at the ouput but when i removed it and it becomes linear the loss starts with a number less than that in sigmoid case 
